...
StartupEvents.registry('palladium:abilities', (event) => {
    ...
    event.create('satsu_iron_man_addon:lightning_2d_shield_circle')
        ...
        .addProperty('random_particles', 'boolean', false, 'Spawn random particles inside the rings')
        .addProperty('push_away', 'boolean', false, 'Push entities away from the ring zone')
                ...
                if (spawnParticles) {
                    ...
                    for (let r = 0; r < ringCount; r++) {
                        ...
                        for (let i = 0; i < ringParticles; i++) {
                            ...
                            let px = centerX + ringRadius * (cosT * rightX + sinT * actualUpX);
                            let py = centerY + ringRadius * (cosT * rightY + sinT * actualUpY);
                            let pz = centerZ + ringRadius * (cosT * rightZ + sinT * actualUpZ);
                        ...
                        if (doRandom) {
                            ...
                            for (let j = 0; j < randomCount; j++) {
                                ...
                                let rpx = centerX + rRight * rightX + rUp * actualUpX + rFwd * viewVec.x();
                                let rpy = centerY + rRight * rightY + rUp * actualUpY + rFwd * viewVec.y();
                                let rpz = centerZ + rRight * rightZ + rUp * actualUpZ + rFwd * viewVec.z();
                    ...
                    if (pushAway) {
                        ...
                        for (let r = 0; r < ringCount; r++) {
                            ...
                            let aabb = new AABB(cx - ringRadius, cy - ringRadius, cz - ringRadius, cx + ringRadius, cy + ringRadius, cz + ringRadius);
