//Originall made by phantompig but edited to copy the palladium property by FSang18

const Vec3 = Java.loadClass("net.minecraft.world.phys.Vec3");

StartupEvents.registry("palladium:abilities", (event) => {
  event
    .create("satsu_iron_man_addon:projectile_with_offset")
    ...
    .addProperty("velocity", "float", 5.0, "The velocity of the projectile")
    .addProperty(
    ...
    .tick((entity, entry, holder, enabled) => {
      if (enabled) {
        ...
        const yawOffset = entry.getPropertyByName("yawOffset");
        const pitchOffset = entry.getPropertyByName("pitchOffset");
        ...
        const ALIGN_VEL_WITH_CALC_ROT = entry.getPropertyByName(
          "align_velocity_with_calculated_rotation",
        );
        ...
        for (let i = 0; i < COUNT; i++) {
          ...
          var yaw = -toRadians(entity.getYaw() + yawOffset + calcYawInacc);
          var pitch = -toRadians(
            entity.getPitch() + pitchOffset + calcPitchInacc,
          );

          if (YAW_OVERRIDE != null) yaw = toRadians(YAW_OVERRIDE);
          if (PITCH_OVERRIDE != null) pitch = toRadians(PITCH_OVERRIDE);

          var x = entity.x + Math.cos(pitch) * Math.sin(yaw),
            y = entity.y + entity.getEyeHeight() + Math.sin(pitch),
            z = entity.z + Math.cos(pitch) * Math.cos(yaw);
